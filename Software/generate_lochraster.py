#!/usr/bin/env python3
"""
generate_lochraster.py — KiCad PCB Generator for Lochraster Prototyping Boards
================================================================================
Date:    2026-03-26
...
Generates 12 KiCad 5-format .kicad_pcb files for perfboard / prototyping PCBs
in three standard sizes (50x70, 70x100, 100x160 mm) and four wiring patterns:

  - unconnected : isolated pads with GND/VCC power rails on outer columns
  - stripboard  : horizontal copper strips on B.Cu (Streifenraster)
  - group_3     : pads connected in groups of 3 on B.Cu, with power rails
  - breadboard  : solderless-breadboard-style column buses on B.Cu

Each board includes:
  - 2.54 mm pitch through-hole pad grid with 1.0 mm drill
  - Rounded-rectangle board outline (Edge.Cuts, 4 mm corner radius)
  - GND / VCC power terminals with oval pads, microvia rings, and copper zones
  - Alternating GND/VCC labels on outer rail columns (F.SilkS + B.SilkS)
  - Branding area with text and raster logos on both silk layers
  - B.SilkS trace-visualisation lines mirroring the B.Cu pattern
  - blank.kicad_wks + .kicad_pro so the page border never leaks into Gerbers

Usage:
  python generate_lochraster.py

Output files land in the current working directory.
After running, use fill_zones.py (via KiCad Python) to fill the copper zones
in the unconnected and group_3 variants.
"""
import os
import math
import uuid
